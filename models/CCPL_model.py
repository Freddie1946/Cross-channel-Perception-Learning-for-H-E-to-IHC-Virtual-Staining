...
from .DCP import DCP_LOSS
from .PCLS import UNet_pro
from .FD import FeatureDistillationLoss
from .NMCC import NMCC_Loss

import os
import timm


os.environ["HF_TOKEN"] = "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"
...
class CCPLModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--thresh_FOD_h', type=float, default=0.15)
        parser.add_argument('--thresh_FOD_d', type=float, default=0.15)
        parser.add_argument('--thresh_mask_h', type=float, default=0.68)
        parser.add_argument('--thresh_mask_d', type=float, default=0.68)
        parser.add_argument('--lamba_FD', type=float, default=1, help='weight for FD loss')
        parser.add_argument('--lamba_NMCC', type=float, default=1, help='weight for NMCC loss')
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_GAN', 'D_real', 'D_fake', 'G', 'NCE', 'DCP', 'ssim', 'FD', 'NMCC']
        ...
        self.tile_encoder = timm.create_model("hf_hub:prov-gigapath/prov-gigapath", pretrained=True) # 新加的初始化
...
#-------------------------------------------#
            ...
            self.criterionDCP = DCP_LOSS(opt.alpha_h,opt.alpha_d,opt.thresh_FOD_h,opt.thresh_FOD_d,opt.thresh_mask_h,opt.thresh_mask_d).to(self.device) #old
            self.criterionFD = FeatureDistillationLoss(self.tile_encoder).to(self.device)
            self.criterionNMCC = NMCC_Loss(self.tile_encoder,opt.alpha_h,opt.alpha_d).to(self.device)
    ...
    def optimize_parameters(self):
        ...
        # for param in self.netG.tile_encoder.parameters():
        #     param.requires_grad = False
    ...
    def forward(self):
        ...
        # self.fake = self.netG(self.real) unetforward
    ...
    def compute_G_loss(self):
        ...
        #############################
        # DCP: Multi-Level Protein Awareness Loss
        # self.loss_DCP, self.mask_A, self.mask_B = self.criterionDCP(self.fake_B,self.real_B) # old
        self.loss_DCP, self.mask_A, self.mask_B, _, _ = self.criterionDCP(self.fake_B,self.real_B) # new 后两个置空
        self.loss_FD = self.criterionFD(self.fake_B,self.real_B)
        self.loss_NMCC = self.criterionNMCC(self.fake_B,self.real_B)
        ...
        self.loss_G = self.loss_G_GAN + loss_NCE_all + self.loss_GP  + 0.05* self.loss_ssim  + self.loss_DCP + self.opt.lamba_FD * self.loss_FD + self.opt.lamba_NMCC *self.loss_NMCC
        return self.loss_G
